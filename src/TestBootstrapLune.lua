...
local function isSpecScript(aScript)
	return aScript:match("%.spec.lua$") ~= nil
end
...
local function getPath(module, root)
	...
	while last ~= nil and last ~= root do
		table.insert(path, stripSpecSuffix(last))
		last = last.Parent
	end
	-- table.insert(path, stripSpecSuffix(root))
...
function TestBootstrap:run(modules, reporter, otherOptions)
	...
	if type(modules) ~= "table" then
		error(("Bad argument #1 to TestBootstrap:run. Expected table, got %s"):format(typeof(modules)), 2)
	end
